...
static psych_int64 PsychOSScheduleSoftSyncFlip(PsychWindowRecordType *windowRecord, double tWhen, psych_int64 targetMSC)
{
    ...
    // fullscreen window. We need this, because we need kms-pageflipping for this to work.
    // Also check if the low-level access support is available, which will be needed:
    ...
    // We only support AMD GPUs at the moment, nothing else:
